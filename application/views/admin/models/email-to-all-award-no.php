@@ -0,0 +1,131 @@
<script>
    function sendEmailToAllAwardNo(templateId, templateSubject) {
        Swal.fire({
            title: 'Please Wait',
        ...
        $.get( "<?=base_url('admin/email/getAllAwardNoEmails/json')?>", function(response) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    sendEmailToAllAwardNoConfirm(templateId);
                }
            });
    ...
    function sendEmailToAllAwardNoConfirm(templateId) {
        Swal.fire({
            title: 'Please Wait',
        ...
        $.get( "<?=base_url('admin/email/getTemplateById/')?>"+templateId, function(response) {
            ...
            if (response.status == 'success')
            {
                ...
                $.post( "<?=base_url('admin/email/sendToAllAwardNo')?>",
                    {
